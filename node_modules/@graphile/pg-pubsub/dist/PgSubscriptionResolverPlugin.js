...
function withInitialEvent(topic, initialEvent) {
    return function subscribeWithInitialEvent(asyncIterator, _parent, args, resolveContext, resolveInfo) {
        return tslib_1.__asyncGenerator(this, arguments, function* subscribeWithInitialEvent_1() {
            const event = yield tslib_1.__await(initialEvent(args, resolveContext, resolveInfo));
            if (event != null) {
                if (typeof event !== "object") {
                    throw new Error("event returned from pgSubscription initialEvent must be an object");
                }
                yield yield tslib_1.__await(Object.assign(Object.assign({}, event), { topic }));
            }
            ...
            */
            try {
                while (true) {
                    const next = yield tslib_1.__await(asyncIterator.next());
                    if (next.done) {
                        return yield tslib_1.__await(next.value);
                    }
                    else {
    ...
    builder.hook("GraphQLObjectType:fields:field", (field, build, graphileContext) => {
        ...
        const subscribe = async (parent, args, resolveContext, resolveInfo) => {
            ...
            if (filter) {
                ...
                const oldAsyncIterator = asyncIterator;
                asyncIterator = graphql_subscriptions_1.withFilter(() => oldAsyncIterator, filter)(parent, args, resolveContext, resolveInfo);
            }
